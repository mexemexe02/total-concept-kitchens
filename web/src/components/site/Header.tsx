...
export function Header() {
  return (
    <header
      className={cn(
        /* Always light chrome: logo colours stay true to the file (no dark bar behind transparent PNG/SVG). */
    ...
    >
      <div className="mx-auto flex min-h-[5.5rem] max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:min-h-[6rem] sm:px-6 sm:py-3.5">
        ...
        >
          {mainNav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-md px-2 py-1.5 text-[0.9375rem] tracking-tight transition hover:bg-stone-900/[0.04] hover:text-bronze"
            >
              {item.label}
            </Link>
          ))}
        ...
        <Link
          href="/contact"
          className="rounded-full bg-charcoal px-4 py-2 text-sm font-semibold text-cream shadow-sm ring-1 ring-charcoal/20 transition hover:bg-bronze hover:ring-bronze/40"
        >
          Book consult
        </Link>
      </div>
      <nav
        className="flex gap-6 overflow-x-auto border-t border-stone-200/80 px-4 py-2.5 text-sm font-medium text-stone-600 md:hidden"
      ...
      >
        {mainNav.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="shrink-0 hover:text-bronze"
          >
            {item.label}
          </Link>
        ))}
